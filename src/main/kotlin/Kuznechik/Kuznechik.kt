...
//    }
    ...
    fun bytesToString(decryptedBytes: ByteArray): List<String> {
        ...
        result =
            decryptedBytes.map { Integer.toHexString(it.toInt() and 0xff) }  // Конвертация каждого байта в шестнадцатеричную строку
    ...
    /**
     * Функция для преобразования массива строк в строку
     *
     * @param decryptedString - массив дешифрованных байтов
     */
    fun arrayToString(decryptedString: List<String>): String {
        // Инициализируем переменную, которая будет хранить результирующую строку
        var resultString = ""
        // Проходимся по всем элементам входного массива
        for (stringByte in decryptedString) {
            // Если текущий элемент равен "0"
            if (stringByte == "0") {
                // Добавляем "0" в начало элемента
                resultString += "0$stringByte"
            } else {
